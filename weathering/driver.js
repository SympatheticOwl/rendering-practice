...
function setupGeometry(geom) {
    ...
    let indices = new Uint16Array(geom.triangles.flat())
...
function findNeighbors(grid, n, currentVertex) {
    const neighbors = [];
    const positions = grid.attributes[0];
    const currentPos = positions[currentVertex];

    const row = Math.round((currentPos[0] + 1) * (n - 1) / 2);
    const col = Math.round((currentPos[2] + 1) * (n - 1) / 2);

    // all potential neighbors for a point in a grid
    ...
    for (const [dx, dy] of adjacentPoints) {
        const newRow = row + dx;
        const newCol = col + dy;

        if (newRow >= 0 && newRow < n && newCol >= 0 && newCol < n) {
            neighbors.push(newRow * n + newCol);
        }
    }
...
function smoothTerrain(grid, n, weatheringPasses) {
    const positions = grid.attributes[0];

    for (let pass = 0; pass < weatheringPasses; pass++) {
        const newHeights = new Float32Array(positions.length);

        for (let i = 0; i < positions.length; i++) {
            const neighbors = findNeighbors(grid, n, i);
            let neighborSum = 0;

            // cumulate neighbor heights to find average
            for (const neighbor of neighbors) {
                neighborSum += positions[neighbor][1];
            }
            const neighborAvg = neighborSum / neighbors.length;

            // 50% weight avg
            newHeights[i] = (positions[i][1] + neighborAvg) / 2;
        }

        // update new heights for y
        for (let i = 0; i < positions.length; i++) {
            positions[i][1] = newHeights[i];
        }
    }
}
...
function generate() {
    ...
    const w = Number(document.querySelector('#weathering').value) || 0
    let grid = makeGrid(n);
    makeFaults(grid, f);

    if (w > 0) {
        smoothTerrain(grid, n, w);
    }
