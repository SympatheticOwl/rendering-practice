...
function setupGeomery() {
    ...
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, ind16Array, gl.STATIC_DRAW);
...
function draw(milliseconds) {
    ...
    let c = Math.cos(milliseconds/250);
    let s = Math.sin(milliseconds/500);
    let x = Math.cos(milliseconds/1000)/2;
    let y = Math.sin(milliseconds/1000)/2;

    let transformMatrix = new Float32Array([c,-s,0,0, s,c,0,0, 0,0,1,0, x,y,0,1]);

    let aUniform = gl.getUniformLocation(program, 'a_matrix');
    gl.uniformMatrix4fv(aUniform, false, transformMatrix);
