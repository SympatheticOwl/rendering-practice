...
function updateParameters() {
    ...
    return {
        ...
        k: 0.0005,        // Very low stiffness
        mu: 0.2,          // Increased viscosity for better settling
        g: 9.8,           // Gravity
        bounds: 1,
        maxSpeed: h/dt,
        maxForce: mass * params.g * 5,
        stackingFactor: 0.1  // How much density increase to allow per layer of stacking
    };
}
...
function computeForces(spheres, neighbors, params) {
    ...
    neighbors.forEach((neighborList, i) => {
        ...
        neighborList.forEach(({j, r, dx, dy}) => {
            ...
            // Calculate pressure force only when particles are too close
            const densityRatio = sphere.density / params.rho0;
            if (densityRatio > 1.0) {
                const pressureForce = Kernels.gradSpiky(r, params.h, dx, dy);

                // Scale pressure force relative to gravitational force
                const pressureMagnitude = sphere.mass * params.k *
                    Math.pow(densityRatio - 1.0, 2);  // Quadratic scaling for better stability

                // Directional force with reduced vertical component
                sphere.force[0] -= pressureMagnitude * pressureForce[0];
                sphere.force[1] -= pressureMagnitude * pressureForce[1] * 0.1; // Significantly reduce vertical pressure
            }
