...
function updateParameters() {
    ...
    return {
        ...
        mass,             // Particle mass
        dt,               // Time step
        rho0: 0.5,        // Target density
        k: 0.05,          // Pressure coefficient (reduced to avoid overwhelming gravity)
        mu: 0.1,          // Viscosity
        g: 9.8,           // Gravity
        bounds: 1,        // Simulation bounds
        maxSpeed: h/dt,   // Maximum allowed particle speed
        maxForce: mass * 50  // Force limit
    };
}
...
function computeForces(spheres, neighbors, params) {
    ...
    neighbors.forEach((neighborList, i) => {
        ...
        neighborList.forEach(({j, r, dx, dy}) => {
            ...
            // Pressure force following SPH formulation from the PDF
            // Only apply pressure when density exceeds rest density
            const densityError = Math.max(0, sphere.density - params.rho0);
            const neighborDensityError = Math.max(0, neighbor.density - params.rho0);

            if (densityError > 0 || neighborDensityError > 0) {
                const pressureForce = Kernels.gradSpiky(r, params.h, dx, dy);
                // Basic pressure force calculation: F = -mass * mass * (p1 + p2)/(2*ρ2) * ∇W
                const pressureMagnitude = sphere.mass * neighbor.mass *
                    (densityError + neighborDensityError) * params.k /
                    (2 * neighbor.density);
